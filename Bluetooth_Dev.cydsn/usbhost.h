...
#warning "Need to clean up a lot of stuff in usbhost.h"
...
#warning "You need to initialize the SPI interface manually when using the STM32F4 platform"
        ...
        static void init() {
                ...
                //UNC SPI.setClockDivider(4); // Set speed to 84MHz/4=21MHz - the MAX3421E can handle up to 26MHz
#endif
        }
#else
        static void init() {
...
void MAX3421e< SPI_SS, INTR >::regWr(uint8_t reg, uint8_t data) {
    ...
    //SPI_PutArray(buffer,2);
    
    SPI_WriteTxData(reg | 0x02);
    SPI_WriteTxData(data); 
    CyDelayUs(SPI_DELAY);
    SS_Write(1);
    return;
    
        //UNC XMEM_ACQUIRE_SPI();
        ...
        //UNC SPI.transfer(reg | 0x02);
        //UNC SPI.transfer(data);
#else
        SPDR = (reg | 0x02);
        while(!(SPSR & (1 << SPIF)));
        SPDR = data;
...
uint8_t* MAX3421e< SPI_SS, INTR >::bytesWr(uint8_t reg, uint8_t nbytes, uint8_t* data_p) {
    ...
    SPI_WriteTxData(reg | 0x02);
            //UNC SPI.transfer(reg | 0x02);
        while(nbytes) {
                //UNC SPI.transfer(*data_p);
            while(SPI_GetTxBufferSize()==4);
            SPI_WriteTxData(*data_p);
            //SPI_ClearRxBuffer();
                nbytes--;
...
uint8_t MAX3421e< SPI_SS, INTR >::regRd(uint8_t reg) {
    ...
    //Old COde below
        ...
        //UNC SPI.transfer(reg);
        //uint8_t rv = 0;//UNC uint8_t rv = SPI.transfer(0); // Send empty byte
        SPI_SS::Set();
#else
        SPDR = reg;
        while(!(SPSR & (1 << SPIF)));
...
uint8_t* MAX3421e< SPI_SS, INTR >::bytesRd(uint8_t reg, uint8_t nbytes, uint8_t* data_p) {
    ...
    SPI_WriteTxData(reg);
            ...
            SPI_ReadRxData();
        while(nbytes) {
            //UNC *data_p++ = SPI.transfer(0);
            SPI_WriteTxData(0);
            while(SPI_GetRxBufferSize()==0);
            *data_p++ = SPI_ReadRxData();
            nbytes--;
        }
        CyDelayUs(SPI_DELAY);
        SS_Write(1);
...
uint8_t MAX3421e< SPI_SS, INTR >::Task(void) {
        ...
        pinvalue = INT_PIN_Read();//INTR::IsSet(); //Read();
